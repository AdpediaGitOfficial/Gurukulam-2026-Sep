...
describe("abbreviating for a tile", () => {
  it("reads crores the way the audience does", () => {
    expect(formatRupeesShort(rupees("13100000"))).toBe("₹1.31 Cr");
    expect(formatRupeesShort(rupees("23230000"))).toBe("₹2.32 Cr");
  });

  it("reads lakhs", () => {
    expect(formatRupeesShort(rupees("4840000"))).toBe("₹48.4 L");
    expect(formatRupeesShort(rupees("100000"))).toBe("₹1 L");
  });

  it("cuts trailing zeros rather than padding them", () => {
    // "₹2 Cr", not "₹2.00 Cr" — the decimals are there to carry information.
    expect(formatRupeesShort(rupees("20000000"))).toBe("₹2 Cr");
    expect(formatRupeesShort(rupees("15000000"))).toBe("₹1.5 Cr");
  });

  it("switches scale exactly at the boundary, not near it", () => {
    expect(formatRupeesShort(rupees("9999999"))).toBe("₹99.99 L");
    expect(formatRupeesShort(rupees("10000000"))).toBe("₹1 Cr");
    expect(formatRupeesShort(rupees("99999"))).toBe("₹99,999");
    expect(formatRupeesShort(rupees("100000"))).toBe("₹1 L");
  });

  it("leaves small amounts exact, because abbreviating them buys nothing", () => {
    expect(formatRupeesShort(rupees("40000"))).toBe("₹40,000");
  ...
  it("truncates rather than rounds up, so a tile never overstates", () => {
    ...
    expect(formatRupeesShort(rupees("13199000"))).toBe("₹1.31 Cr");
  });
  ...
  it("drops the symbol when asked", () => {
    expect(formatRupeesShort(rupees("13100000"), { symbol: false })).toBe("1.31 Cr");
  });
});
